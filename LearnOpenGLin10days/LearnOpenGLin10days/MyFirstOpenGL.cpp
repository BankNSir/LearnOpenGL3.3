...
int main()
{
	...
	/*shadowTransforms.push_back(shadowBufferProjectionMatrix * camera.GetViewMatrix());
	shadowTransforms.push_back(shadowBufferProjectionMatrix * camera.GetViewMatrix());
	shadowTransforms.push_back(shadowBufferProjectionMatrix * camera.GetViewMatrix());
	shadowTransforms.push_back(shadowBufferProjectionMatrix * camera.GetViewMatrix());
	shadowTransforms.push_back(shadowBufferProjectionMatrix * camera.GetViewMatrix());
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, depthCubeMap);
		pointLightShadowMapScene.use();
		pointLightShadowMapScene.setMat4("view", view);
		pointLightShadowMapScene.setMat4("projection", projection);
		pointLightShadowMapScene.setInt("diffuseTexture", 0);
		pointLightShadowMapScene.setInt("depthMap", 1);
		pointLightShadowMapScene.setVec3("lightPos", lightPos);
		pointLightShadowMapScene.setVec3("viewPos", camera.Position);
		pointLightShadowMapScene.setFloat("far_plane", shadowBufferFarPlane);

		RenderScene(pointLightShadowMapScene);
