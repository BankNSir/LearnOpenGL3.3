...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		RenderScene(shadowmapDepthShader);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		testsceneShader.use();
		testsceneShader.setMat4("view", view);
		testsceneShader.setMat4("projection", projection);
		testsceneShader.setInt("texture1", 0);
		glBindTexture(GL_TEXTURE_2D, woodTexture);
		RenderScene(testsceneShader);*/
		...
		//// ConfigureShaderAndMatrices
		//glBindTexture(GL_TEXTURE_2D, depthMap);
		//shadowmapQuadShader.use();
		//shadowmapQuadShader.setInt("depthMap", 0);
		//RenderQuad();

		// 2nd pass scene
		glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);
		shadowmapScene.use();
		// 1. bind all uniforms 1.1 MVP 1.2 lightspaceMat/ 1.3 diffuse/ 1.4 shadowMap/ 1.5 lightPos/ 1.6 viewPos/
		shadowmapScene.setMat4("lightSpaceMatrix", lightSpaceMatrix);
		// texture
		shadowmapScene.setInt("diffuseTexture", 0);
		shadowmapScene.setInt("shadowMap", 1);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, woodTexture);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, depthMap);
		shadowmapScene.setVec3("lightPos", glm::vec3(-2.0f, 4.0f, -1.0f));
		shadowmapScene.setVec3("viewPos", camera.Position);
		// view projection
		glm::mat4 view = camera.GetViewMatrix();
		glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / SCR_HEIGHT, nearPlane, farPlane);
		shadowmapScene.setMat4("view", view);
		shadowmapScene.setMat4("projection", projection);
		RenderScene(shadowmapScene);
