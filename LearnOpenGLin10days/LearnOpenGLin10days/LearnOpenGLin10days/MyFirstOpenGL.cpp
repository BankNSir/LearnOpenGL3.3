...
int main()
{
	...
	// SSAO
	Shader SSAOgBufferShader("SSAOgBuffer.vs", "SSAOgBuffer.fs");
	Shader SSAOalgoShader("SSAOalgo.vs", "SSAOalgo.fs");
	Shader SSAOlightingShader("SSAOlighting.vs", "SSAOlighting.fs");
	...
	// noiseRotationTexture setup
	std::uniform_real_distribution<float> randomFloats(0.0, 1.0); // random floats between 0.0 - 1.0
	std::default_random_engine generator;
	std::vector<glm::vec3> SSAOKarnel;

	// create 64 karnel direction vector
	for (int i = 0; i < 64; i++)
	{
		glm::vec3 direction;
		direction.x = randomFloats(generator) * 2.0f - 1.0f;
		direction.y = randomFloats(generator) * 2.0f - 1.0f;
		direction.z = randomFloats(generator);

		direction = glm::normalize(direction);

		float magnitute = randomFloats(generator);
		glm::vec3 karnel = magnitute * direction;

		float scale = (float)i / 64.0f;
		scale = lerp(0.1f, 1.0f, scale * scale);
		karnel *= scale;
		SSAOKarnel.push_back(karnel);
	}

	// create random Rotation vector
	vector<glm::vec3> SSAONoiseRotation;
	for (int i = 0; i < 16; i++)
	{
		glm::vec3 randomRotation = glm::vec3(randomFloats(generator) * 2.0f - 1.0f, randomFloats(generator) * 2.0f - 1.0f, 0.0f);
		SSAONoiseRotation.push_back(randomRotation);
	}


	// create 4X4 noise Texture
	unsigned int noiseRotationTexture;
	glGenTextures(1, &noiseRotationTexture);
	glBindTexture(GL_TEXTURE_2D, noiseRotationTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, 4, 4, 0, GL_RGB, GL_FLOAT, &SSAONoiseRotation[0]);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		SSAOgBufferShader.use();
		/*uniform sampler2D diffuseTexture;
		uniform sampler2D specularTexture;*/
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, container2Texture);
		SSAOgBufferShader.setInt("diffuseTexture", 0);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, container2Texture_specular);
		SSAOgBufferShader.setInt("specularTexture", 1);

		SSAOgBufferShader.setMat4("view", view);
		SSAOgBufferShader.setMat4("projection", projection);

		RenderDeferredShadingScene(SSAOgBufferShader);

		glBindFramebuffer(GL_FRAMEBUFFER, 0);

		// 2nd pass calculate SSAO from 1st-pass gBuffer
		//glBindFramebuffer(GL_FRAMEBUFFER, SSAOFBO); // fin
		// 2.1 create SSAO FRAMEBUFFER and bind it //fin
		// 2.2 write SSAOshader w/ SSAO algo(karnel hemisphere) //current work
		// 2.3 render SSAO scene
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, gPos);
		SSAOalgoShader.setInt("positionTexture", 0);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, gNormal);
		SSAOalgoShader.setInt("normalTexture", 1);
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, noiseRotationTexture);
		SSAOalgoShader.setInt("noiseRotationTexture", 2);
		for (int i = 0; i < 64; i++)
		{
			SSAOalgoShader.setVec3("samples[" + std::to_string(i) + "]", SSAOKarnel[i]);
		}

		SSAOalgoShader.setMat4("projection", projection);
		...
		// ___________________________________________________ 3rd pass
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, gPos);
		SSAOlightingShader.setInt("positionTexture", 0);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, gNormal);
		SSAOlightingShader.setInt("normalTexture", 1);
		glActiveTexture(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, gAlbedoSpec);
		SSAOlightingShader.setInt("albedoSpecTexture", 2);
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D, SSAOTexture);
		SSAOlightingShader.setInt("ambientOcclusion", 3);

		SSAOlightingShader.setVec3("viewPos", camera.Position);

		for (int i = 0, n = lightPositions.size(); i < n; i++)
		{
			SSAOlightingShader.setVec3("lights[" + std::to_string(i) + "].LightPos", lightPositions[i]);
			SSAOlightingShader.setVec3("lights[" + std::to_string(i) + "].LightColor", lightColors[i]);
			SSAOlightingShader.setFloat("lights[" + std::to_string(i) + "].Radius", lightRadius[i]);
		}

		SSAOlightingShader.setFloat("exposure", exposure);
