...
{            
	...
	// hdr
	result += PhongCalculation(lights[0]);
...
vec3 PhongCalculation(Light light)
{
    ...
    vec3 normal = NORMAL;
    // ambient
    vec3 ambient = 0.5f * texture(ambientOcclusion, fs_in.TexCoords).r * COLOR;
    // diffuse
    ...
    return ambient + 0 * (diffuse * color + specular * SPECULAR)/(distance * distance);    
}
